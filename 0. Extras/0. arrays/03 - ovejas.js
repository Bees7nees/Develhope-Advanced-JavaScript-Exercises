...
Si el lobo es el animal más cercano a ti, devuelve "Por favor, ¡vete y deja de comer mis ovejas!". De lo contrario, devuelve "¡Oye! ¡Oveja número N! 
¡Estás a punto de ser comida por un lobo!" donde N es la posición de la oveja en la fila.
...
function encontrarLobo(fila) {
  ...
  fila.forEach((animal, index) => {
    if (animal === "lobo" && index === (fila.length - 1)) {
      return "Vete y deja de comer mis ovejas!";
    }
    else if (animal === "lobo") {
      return `Oveja número ${index + 1}! ¡Estás a punto de ser comida por un lobo!`;
    }
  });
}
